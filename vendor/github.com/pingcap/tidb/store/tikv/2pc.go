...
func (c *twoPhaseCommitter) doActionOnKeys(bo *Backoffer, action twoPhaseCommitAction, keys [][]byte) error {
	...
	if action == actionCommit {
		...
		twoPhaseCommitGP.Go(func() {
			...
			if e != nil {
				log.Infof("con:%d 2PC async doActionOnBatches %s err: %v", c.connID, action, e)
...
func (c *twoPhaseCommitter) doActionOnBatches(bo *Backoffer, action twoPhaseCommitAction, batches []batchKeys) error {
	...
	for i := 0; i < len(batches); i++ {
		if e := <-ch; e != nil {
			log.Infof("con:%d 2PC doActionOnBatches %s failed: %v, tid: %d", c.connID, action, e, c.startTS)
			// Cancel other requests and return the first error.
			if cancel != nil {
				log.Infof("con:%d 2PC doActionOnBatches %s to cancel other actions, tid: %d", c.connID, action, c.startTS)
				cancel()
			}
			if err == nil {
				err = e
...
func (c *twoPhaseCommitter) commitSingleBatch(bo *Backoffer, batch batchKeys) error {
	...
	if keyErr := commitResp.GetError(); keyErr != nil {
		...
		log.Infof("2PC failed commit primary key: %v, retry later, tid: %d", err, c.startTS)
...
func (c *twoPhaseCommitter) cleanupSingleBatch(bo *Backoffer, batch batchKeys) error {
	...
	if keyErr := resp.BatchRollback.GetError(); keyErr != nil {
		...
		log.Infof("2PC failed cleanup key: %v, tid: %d", err, c.startTS)
		return errors.Trace(err)
	}
	return nil
}
...
func (c *twoPhaseCommitter) execute(ctx context.Context) error {
	...
	if err != nil {
		log.Infof("con:%d 2PC failed on prewrite: %v, tid: %d", c.connID, err, c.startTS)
		return errors.Trace(err)
	}
	...
	if err != nil {
		...
		if !c.mu.committed {
			log.Infof("con:%d 2PC failed on commit: %v, tid: %d", c.connID, err, c.startTS)
			return errors.Trace(err)
		}
		log.Infof("con:%d 2PC succeed with error: %v, tid: %d", c.connID, err, c.startTS)
	}
	return nil
}
